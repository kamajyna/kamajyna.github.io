...
7. Frontmatter에 대표 썸네일 이미지 URL을 다음 형식으로 작성할 것: `image: "https://pollinations.ai/p/[핵심_영어_키워드]?width=800&height=450"`. 여기서 '[핵심_영어_키워드]'는 본 포스트 주제를 나타내는 2~3개의 영어 단어(예: artificial_intelligence, server_room 등)를 쉼표나 언더스코어로 연결해서 넣어주세요. 절대 본문(내용) 안에는 이미지를 마크다운 문법으로 중복 삽입하지 마세요.
...
tags: [AI, 기술, 트렌드]
image: "https://pollinations.ai/p/artificial_intelligence,future?width=800&height=450"
---
...
6. Frontmatter에 대표 썸네일 이미지 URL을 다음 형식으로 작성할 것: `image: "https://pollinations.ai/p/[해당종목_관련_영어_키워드]?width=800&height=450"`. 여기서 '[해당종목_관련_영어_키워드]'는 해당 기업의 실제 비즈니스 모델이나 주요 제품을 잘 보여주는 구체적인 영어 단어 2~3개(예: Coca_Cola_beverage, Real_estate_skyscraper, Apple_device_modern 등)를 언더스코어로 연결해서 사용하세요. 단순한 'stock_chart'와 같은 키워드는 피하여 썸네일들이 서로 비슷해 보이지 않게 하세요. 절대 본문(내용) 안에는 이미지를 마크다운 문법으로 중복 삽입하지 마세요.
...
image: "https://pollinations.ai/p/Apple_device_modern_design?width=800&height=450"
---
...
def save_post(content, category):
    ...
    image_url_match = re.search(r'^image:\s*"(https?://pollinations\.ai/p/[^"]+)"', content, re.MULTILINE)
    if image_url_match:
